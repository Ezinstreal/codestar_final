...
<form style="padding: 20px" action=" {{route('admin.posts.update',$post->id)}} " method="POST">
    @method('PUT')
    @csrf
    <table class="table">
            ...
            <form id="form" action="{{ route('admin.posts.store') }}" method="POST">
                ...
                <div class="form-group">
                    ...
                    <select class="form-control" name="user">
                        ...
                        @foreach ($author as $item)
                            <option {{ $post->users->first()->id == $item->id ? 'selected': '' }} value="{{ $item->id }}">{{ $item->name }}</option>
                        @endforeach
                    </select>
                  </div>
                ...
                <div class="form-group"><label for="">Options</label>
                    <div class="form-check form-check-inline">
                        <input {{ $post->type == 1 ? 'checked': '' }} class="form-check-input" type="radio" name="type" required="required" id="inlineRadio1" value="1">
                        <label class="" for="inlineRadio1">Bai viet </label>
                        </div>
                        <div class="form-check form-check-inline">
                        <input {{ $post->type == 2 ? 'checked': '' }} class="form-check-input" type="radio" name="type" required="required" id="inlineRadio2" value="2">
                        <label class="" for="inlineRadio2"> Nhap</label>
                        </div>
                    <div class="form-check form-check-inline">
                        <input {{$post->status == 1 ? 'checked' : ''}} type="checkbox" name="status" value="1" class="form-check-input" id="status1">
                        <label class="form-check-label" for="status">Trạng thái</label>
                    </div>
                    <div class="form-check form-check-inline">
                        <input type="checkbox" {{$post->status2 == 1 ? 'checked' : ''}} name="status2" value="1" class="form-check-input" id="status2">
                ...
                <textarea id="summernote" name="content" class="@error('content') border border-danger @enderror">{!! $post->content !!}</textarea>
