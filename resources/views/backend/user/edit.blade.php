...
<div style="padding: 20px">
    <form action="{{ route('admin.user.update',$data->id) }}" method="POST">
        ...
        <div class="form-floating mb-3">
            ...
            <input type="text" class="form-control @error('name') border border-danger @enderror" id="floatingInput" placeholder="Username" name="name" value="{{ $data->name }}">
        </div>
        <div class="form-floating mb-3">
            <label for="floatingInput">Email address</label>
            @error('email')<span class="text-danger">{{ $message }}</span>@enderror
            <input type="email" class="form-control @error('email') border border-danger @enderror" id="floatingInput" placeholder="name@example.com" name="email" value="{{ $data->email }}" readonly>
        ...
        <div class="form-check form-check-inline">
            <input required="required" class="form-check-input" type="radio" name="role" id="inlineRadio1" value="1" {{ $data->roles->first()->id == 1 ? 'checked':'' }}>
            <label class="form-check-label" for="inlineRadio1">admin</label>
        </div>
        @endcan
        <div class="form-check form-check-inline">
            <input required="required" class="form-check-input" type="radio" name="role" id="inlineRadio2" value="2" {{ $data->roles->first()->id == 2 ? 'checked':'' }}>
            <label class="form-check-label" for="inlineRadio2">author</label>
        </div>
        <div class="form-check form-check-inline">
            <input required="required" class="form-check-input" type="radio" name="role" id="inlineRadio2" value="3" {{ $data->roles->first()->id == 3 ? 'checked':'' }}>
        ...
        <div class="form-floating mb-3">
            ...
            <input type="text" class="form-control" id="floatingInput" placeholder="Phone" name="phone" value="{{ $profile->phone }}">
        </div>
        <div class="form-floating mb-3">
            <label for="floatingInput">Address</label>
            <input type="text" class="form-control" id="floatingInput" placeholder="Address" name="address" value="{{ $profile->address }}">
        </div>
        <div class="form-floating mb-3">
            <label for="floatingInput">Link</label>
            <input type="text" class="form-control" id="floatingInput" placeholder="Link" name="link" value="{{ $profile->link }}">
        </div>
